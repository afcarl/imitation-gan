...
class Critic(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.rnn = nn.GRU(input_size=opt.emb_size, hidden_size=opt.critic_hidden_size,
                          ...
                          batch_first=True)
        self.dist1 = nn.Linear(opt.critic_hidden_size, opt.emb_size)
        self.dist2 = nn.Linear(opt.emb_size, opt.vocab_size)
        self.embedding.weight = self.dist2.weight  # tie weights
    ...
    def forward(self, actions):
        ...
        flat_costs = -F.log_softmax(self.dist2(self.dist1(flattened)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--actor_entropy_reg', type=float, default=1.0,
                        help='policy entropy regularization')
    parser.add_argument('--critic_entropy_reg', type=float, default=1e-2,
    ...
    for cur_iter in xrange(opt.niter):
        ...
        for actor_i in xrange(actor_iters):
            ...
            loss -= opt.actor_entropy_reg * entropy
            loss.backward()
